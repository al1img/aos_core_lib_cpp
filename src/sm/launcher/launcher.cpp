    ...
    const Array<cloudprotocol::EnvVarsInstanceInfo>& envVarsInfo, Array<cloudprotocol::EnvVarsInstanceStatus>& statuses)
{
    ...
    for (const auto& envVarInfo : envVarsInfo) {
        if (auto err = statuses.EmplaceBack(cloudprotocol::EnvVarsInstanceStatus {envVarInfo.mFilter, {}});
            !err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }

        auto [currentStatus, err] = statuses.Back();
        if (!err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }
        ...
        for (const auto& envVar : envVarInfo.mVariables) {
            ...
            if (envVar.mTTL.HasValue() && *envVar.mTTL < now) {
                ...
                if (err = currentStatus.mStatuses.EmplaceBack(cloudprotocol::EnvVarStatus {envVar.mName, envVarErr});
                    !err.IsNone()) {
                    return AOS_ERROR_WRAP(err);
                }
            } else {
...
Error Launcher::GetEnvChangedInstances(Array<InstanceData>& instances) const
{
    for (auto& instance : mCurrentInstances) {
        ...
        if (auto err = instances.PushBack(InstanceData {instance.Info(), instance.InstanceID()}); !err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }
    }
...
Error Launcher::SendEnvChangedInstancesStatus(const Array<InstanceData>& instances)
{
    ...
    for (const auto& instanceData : instances) {
        auto [instance, err] = GetInstance(instanceData.mInstanceID);
        if (!err.IsNone()) {
            LOG_ERR() << "Can't get instance: instanceID=" << instanceData.mInstanceID << ", err=" << err;
        ...
        if (err = status->PushBack({instance->Info().mInstanceIdent, instance->GetServiceVersion(),
                instance->RunState(), instance->RunError()});
            !err.IsNone()) {
            return AOS_ERROR_WRAP(err);
        }
